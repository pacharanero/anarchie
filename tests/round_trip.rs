...
use anarchie::rm::{Composition, ContentItem, DataValue, Item, ItemStructure, PartyProxy, Text};

const BLOOD_PRESSURE: &str = include_str!("fixtures/blood-pressure-composition.json");

fn parse(json: &str) -> Composition {
    anarchie::rm::from_canonical_str(json).expect("fixture should parse as COMPOSITION")
}

fn canonical(composition: &Composition) -> String {
    anarchie::rm::to_canonical_string(composition).expect("serialisation should succeed")
}
