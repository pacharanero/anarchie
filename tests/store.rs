...
use anarchie::rm::Composition;
use anarchie::store::{Audit, ChangeType, Deployment, DeploymentConfig};
...
fn fixture() -> Composition {
    ...
    anarchie::rm::from_canonical_str(&json).expect("parse fixture")
}
...
fn commit_assigns_a_version_uid_and_writes_head_to_the_working_tree() {
    ...
    let deployment = Deployment::init(tmp.path(), DeploymentConfig::new("anarchie.test")).unwrap();
    let repo = deployment
        .create_ehr(&audit(ChangeType::Creation, "Create EHR"))
        .unwrap();
    ...
    let outcome = repo
        .commit_composition(
            fixture(),
            None,
            &audit(ChangeType::Creation, "Admission vitals"),
        )
        .unwrap();
...
fn audit_description_becomes_the_commit_subject() {
    ...
    let described = repo
        .commit_composition(
            fixture(),
            None,
            &audit(ChangeType::Creation, "Admission observations"),
        )
        .unwrap();
    ...
    assert_eq!(
        history[0].subject,
        "Update composition {} (v2)".replace("{}", &described.object_id)
    );
