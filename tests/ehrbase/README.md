...
The differential test fetches EHRbase's Apache-2.0 `patient_blood_pressure.v1` test OPT from the pinned EHRbase 2.34.0 source revision into a temporary directory, verifies its SHA-256, and does not redistribute it. It derives one canonical Composition from anarchie's synthetic blood-pressure fixture: the server-assigned UID is omitted, the template and blood-pressure archetype identifiers are changed to match the OPT, and the nested OBSERVATION gains the `archetype_details` required by the RM invariant. That exact derived Composition is loaded into both systems. The 23 cases in `queries.json` compare normalised column names and rows covering: COUNT, path projection (single and multi-column), all comparison operators (=, !=, <, <=, >, >=), boolean logic (AND, OR, NOT), MATCHES, equality on text values, ORDER BY (ASC and DESC on identified paths), LIMIT, LIMIT+OFFSET, and all five aggregates (MIN, MAX, SUM, AVG, COUNT). Response metadata and row order are intentionally excluded from comparison.

Known divergences excluded from the corpus: EHRbase does not support `LIKE` on leaf paths or `EXISTS` in WHERE; EHRbase does not support `ORDER BY` aliases (only identified paths); EHRbase does not expose `c/name/value` or `e/ehr_id/value` as queryable AQL paths. These are documented EHRbase limitations, not anarchie bugs.
