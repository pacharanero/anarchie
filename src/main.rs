...
use anarchie::rm::{Composition, ContentItem, DataValue, Item, ItemStructure};
use anarchie::store::{Audit, ChangeType, Deployment, DeploymentConfig, StoreError};
use anarchie::validate::{Opt, ValidationReport};
...
fn validate(file: &PathBuf, template_id: Option<&str>, json: bool) -> Result<()> {
    ...
    let report = anarchie::validate::validate(&composition, opt.as_ref());
    if json {
...
fn index_cmd(rebuild: bool) -> Result<()> {
    ...
    let mut index = anarchie::query::Index::open(&db).context("opening index")?;
    let count = index
...
fn parse_params(pairs: &[String]) -> Result<anarchie::query::Params> {
    let mut params = anarchie::query::Params::new();
    for pair in pairs {
        let (name, value) = pair
            .split_once('=')
...
fn run_aql(deployment: &Deployment, aql: &str, params: &anarchie::query::Params) -> Result<()> {
    let index = anarchie::query::Index::open(index_db_path(deployment)).context("opening index")?;
    let result = anarchie::query::execute(&index, aql, params).context("executing AQL")?;
    ...
    match command {
        QueryCommand::Add {
            ...
            let aql =
                fs::read_to_string(&file).with_context(|| format!("reading {}", file.display()))?;
            let (name, version) =
                anarchie::query::stored::add(deployment.root(), &name, version.as_deref(), &aql)
                    .context("registering stored query")?;
            println!("Registered query {name}/{version}");
            Ok(())
        }
        QueryCommand::List => {
            for q in anarchie::query::stored::list(deployment.root())
                .context("listing stored queries")?
            {
        ...
        QueryCommand::Run {
            ...
            let aql = anarchie::query::stored::get(deployment.root(), &name, version.as_deref())
...
fn serve_cmd(addr: &str) -> Result<()> {
    let deployment = open_deployment()?;
    anarchie::serve::serve(deployment, addr).context("running REST server")
}
...
fn pack_cmd(command: PackCommand) -> Result<()> {
    match command {
        ...
        PackCommand::List => {
            ...
            for name in anarchie::store::bundled_packs() {
                println!("  - {name}");
            }
            Ok(())
        }
...
fn mcp_cmd() -> Result<()> {
    let deployment = open_deployment()?;
    anarchie::serve::run_mcp(deployment).context("running MCP server")
}
...
fn load(file: &PathBuf) -> Result<Composition> {
    ...
    anarchie::rm::from_canonical_str(&json)
...
fn canonicalise(file: &PathBuf) -> Result<()> {
    let composition = load(file)?;
    print!("{}", anarchie::rm::to_canonical_string(&composition)?);
    Ok(())
}

fn composer_name(composition: &Composition) -> Option<String> {
    use anarchie::rm::PartyProxy::*;
...
fn event_data(event: &anarchie::rm::Event) -> &ItemStructure {
    match event {
        anarchie::rm::Event::PointEvent(e) => &e.data,
        anarchie::rm::Event::IntervalEvent(e) => &e.data,
    }
}
