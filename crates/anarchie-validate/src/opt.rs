...
use anarchie_aom::{CAttribute, CComplexObject, CObject};
use anarchie_opt::Opt;
                    ...
                    if let (Some(range), Some(mag)) = (&item.magnitude, magnitude) {
                        if !range.contains(mag) {
                            report.error(
                                ...
                                "C_DV_QUANTITY",
                                format!("magnitude {mag} outside permitted range for units \"{units}\""),
                            );
                        }
                    }
                }
        ...
        CObject::String(cs) => {
            ...
            if !cs.list.is_empty() && !cs.list.iter().any(|s| s == text) {
                report.error(
                    path,
                    "C_STRING",
                    format!("value \"{text}\" not in permitted set: {}", cs.list.join(", ")),
                );
            }
        }
...
fn coded_value(value: &Value) -> Option<(&str, &str)> {
    ...
    let terminology = code
        ...
        .and_then(Value::as_str)
        .unwrap_or("");
    let code_string = code.get("code_string").and_then(Value::as_str).unwrap_or("");
