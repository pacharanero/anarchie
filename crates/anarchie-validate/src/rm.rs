...
use anarchie_rm::{
    Action, AdminEntry, CodePhrase, Composition, ContentItem, Cluster, DataValue, Element,
...
fn validate_observation(obs: &Observation, path: &str, report: &mut ValidationReport) {
    ...
    for (i, event) in obs.data.events.iter().enumerate() {
        ...
        let event_path = format!("{path}/data[{}]/events[{}]", obs.data.archetype_node_id, node_id);
        let _ = i;
...
fn validate_instruction(instr: &Instruction, path: &str, report: &mut ValidationReport) {
    ...
    for activity in &instr.activities {
        validate_item_structure(
            ...
            &format!("{path}/activities[{}]/description", activity.archetype_node_id),
            report,
        );
    }
}
...
fn validate_element(element: &Element, path: &str, report: &mut ValidationReport) {
    ...
    match (&element.value, &element.null_flavour) {
        ...
        (None, Some(null)) => {
            check_code_phrase(&null.defining_code, path, "null_flavour/defining_code", report)
        }
    }
}
...
fn validate_data_value(value: &DataValue, path: &str, report: &mut ValidationReport) {
    ...
    match value {
        ...
        DataValue::DvOrdinal(o) => {
            check_code_phrase(&o.symbol.defining_code, path, "value/symbol/defining_code", report);
        }
        DataValue::DvScale(s) => {
            check_code_phrase(&s.symbol.defining_code, path, "value/symbol/defining_code", report);
        }
        _ => {}
    }
}
