...
use anarchie_query::{execute, stored, Index, Params};
use anarchie_rm::Composition;
use anarchie_store::{Audit, ChangeType, CommitOutcome, Deployment, StoreError};
use anarchie_validate::ValidationReport;
...
impl From<anarchie_query::QueryError> for ApiError {
    fn from(err: anarchie_query::QueryError) -> Self {
        use anarchie_query::QueryError::*;
        match err {
...
pub fn commit_composition(
    ...
    let composition: Composition = anarchie_rm::from_canonical_str(body)
...
mod tests {
    ...
    const BP: &str =
        include_str!("../../anarchie-rm/tests/fixtures/blood-pressure-composition.json");

    fn fresh_deployment() -> (tempfile::TempDir, Deployment) {
        let tmp = tempfile::tempdir().unwrap();
        let dep = Deployment::init(
            tmp.path(),
            anarchie_store::DeploymentConfig::new("test.local"),
        )
        .unwrap();
