...
impl Deployment {
    ...
    fn check_composition(
        &self,
        repo: &crate::deployment::EhrRepo,
        object_id: &str,
    ) -> Result<Vec<String>> {
        let json = repo.cat_head(object_id)?;
        let composition: Composition = match anarchie_rm::from_canonical_str(&json) {
            Ok(c) => c,
        ...
        Ok(report
            .violations
            .iter()
            .filter(|v| v.severity == anarchie_validate::Severity::Error)
