...
- [~] **K4 - Secure package format.** `anarchie pack build`, `inspect`, and `verify` create and validate deterministic data-only `.tar.zst` archives with fixed tar metadata, declared package identity, SHA-256 content checksums, canonical safe-path/link rejection, compressed/expanded archive limits, and atomic build output. `pack install` verifies once, stages only declared content, and atomically materialises it under `knowledge/packages/sha256-<archive-digest>/`; repeated installation is idempotent and failed installation leaves no package material. Activation, removal, rollback, and coexistence policy remain.
