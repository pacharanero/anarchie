...
- [x] **K4 - Secure package format.** `anarchie pack build`, `inspect`, and `verify` create and validate deterministic data-only `.tar.zst` archives with fixed tar metadata, declared package identity, SHA-256 content checksums, canonical safe-path/link rejection, compressed/expanded archive limits, and atomic build output. `pack install` verifies once, retains the verified archive with only its declared material, and atomically materialises both under `knowledge/packages/sha256-<archive-digest>/`; repeated installation is idempotent and failed installation leaves no package material. `pack installed` and read-only `pack audit` enumerate the package store and detect archive or material tampering. Activation, removal, rollback, and coexistence policy are deliberately K8 work because they require historical template-digest protection.
- [~] **K5 - Published International package.** `anarchie pack ckm <checkout> --manifest <knowledge.toml> --version <release> <archive>` deterministically publishes a closed, policy-resolved `ckm-international` source package. It copies only locked artefacts, rechecks source checksums, and includes source evidence, lock, resolution, and machine-readable inclusion/exclusion report. Building and embedding the complete eligible International snapshot in releases remains.
