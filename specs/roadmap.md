...
- [ ] **Stage 3 - `openehr-its`.** Retire the hand-rolled OPT XML importer in `src/opt.rs` for the crate's `opt14` model, then take `flat`/`webtemplate` to deliver the **Renderer formats** and **Explorer interoperability** items below. **Blocked on licence:** the crate is now `BUSL-1.1 AND Apache-2.0`. The separable-features request on [FerroEHR#2807](https://github.com/rubentalstra/FerroEHR/issues/2807) remains necessary but is no longer sufficient. Until the licence changes, the renderer-format work is ours to write or to leave undone - decide which before promising Explorer support.
