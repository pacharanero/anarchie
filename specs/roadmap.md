...
Adopt the published [FerroEHR](https://github.com/rubentalstra/FerroEHR) `openehr-*` crates as the model layer instead of hand-writing and extracting our own. They are generated from the openEHR BMM and carry no database, network, filesystem, or async-runtime dependency, so file-based persistence is unaffected. The rationale, admission criteria, risks, and reversal path are in [rust-sdk.md](rust-sdk.md). **The crate set was relicensed after 0.0.56 and this now decides which stages are open.** `openehr-base`/`-rm`/`-am`/`-lang`/`-term` are Apache-2.0 and remain usable; `openehr-query`, `openehr-adl` and `openehr-its` are BUSL-1.1, which AGPL-3.0-or-later cannot carry. Each stage is independently shippable. Stage 1 has landed: AQL is now parsed by `openehr-query` and narrowed to the executable subset by `src/query/aql/lower.rs`, at a cost of 14 transitive crates, +13% binary size, and a 1.96 MSRV. That measurement is the yardstick for the stages below. It is pinned at 0.0.56, the last MIT release, and Dependabot ignores it.
...
- [ ] **Stage 3 - `openehr-its`.** Retire the hand-rolled OPT XML importer in `src/opt.rs` for the crate's `opt14` model, then take `flat`/`webtemplate` to deliver the **Renderer formats** and **Explorer interoperability** items below. **Confirmed deliberate, generated/hand-written split - partially recoverable.** The maintainer stated the rule (generated code Apache-2.0, hand-written code BUSL-1.1) and committed to splitting `openehr-its` along it, tracked via a GitHub issue he has yet to open. Once it lands, the generated codec/`opt14` half should become usable; `flat`/`webtemplate` are hand-written and will stay BUSL-1.1, so **Explorer/renderer-format work remains ours to write regardless**. The separable-features request on [FerroEHR#2807](https://github.com/rubentalstra/FerroEHR/issues/2807) is still needed on top of the licence split. Re-verify the actual crate boundary when it ships.
- [ ] **Stage 4 - `openehr-am` / `openehr-adl`.** Only when ADL2/OPT2 becomes real work. `openehr-am` is Apache-2.0; `openehr-adl` is BUSL-1.1 and unusable.
- [ ] **Re-read the licence before each stage.** The set moved MIT -> Apache-2.0 -> BUSL-1.1 within a fortnight, per-crate rather than uniformly. Check the licence of the exact version being adopted, not the set's reputation.
