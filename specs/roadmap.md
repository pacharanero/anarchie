...
- [~] **Batteries-included knowledge packages.** The seed exists as eight embedded starter templates, default installation, `--minimal`, local-directory packs, attribution, template validation, and deterministic CKM source inventory through `anarchie knowledge inventory`. The target is the complete eligible published International CKM archetype library plus a conformance-tested executable template catalogue, managed through `knowledge.toml` and `knowledge.lock`; deterministic resolution, secure archives, compiled templates, version-safe activation, registry updates, and release gates remain. See [knowledge-packages.md](knowledge-packages.md).
