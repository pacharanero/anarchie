...
- [ ] **C12 - Run against both community conformance instruments, not one.** As of 2026-09, at least three uncoordinated conformance efforts exist beside the Foundation's own stalled [openEHR/specifications-CNF](https://github.com/openEHR/specifications-CNF) (2017-, `specifications-CNF` PR #5 open unmerged since 2023): Pablo Pazos / CaboLabs's [conformance verification framework](https://github.com/ppazos/openehr-conformance-verification) (spec + datasets + Groovy suite, 2023-2024, dormant since) and [Veredictum](https://veredictum.eu/) (Rust instrument, 2026-, owned by Vernum Projecten B.V.). Treat neither as *the* oracle:
  - Run a released Veredictum catalogue against a disposable anarchie deployment through a version-pinned IXIT configuration, retaining the exchanges and verdict as evidence.
  - Run Pazos's conformance datasets and test specification against the same deployment where its fixtures are usable standalone (its suite is Groovy/Gradle and largely dormant since March 2024; expect to translate fixtures rather than run its harness).
  - **Do not describe either as "independent."** Veredictum's own pitch is "no server grades its own homework", but Vernum Projecten B.V. is simultaneously the commercial licensor of FerroEHR (the CDR it certifies) and the owner of Veredictum (the instrument certifying it) - the same entity holds both ends. Record results as "N/M cases pass against catalogue X, evidence at commit Y", never as a certification or an appeal to either party's authority.
  - Attribute every failure against the cited openEHR specification text before changing anarchie, either catalogue, or a runner. A disagreement between the two catalogues on the same behaviour is itself a finding worth recording, not a tiebreak to resolve informally.
  - Neither instrument is ever a shipped dependency or a build-time requirement; both run test-time only, same as Archie and EHRbase.
- [ ] **C13 - Reassess when/if openEHR publishes a single owned conformance suite.** Marcus Baw (openEHR Software Program Board) has raised with the SPB that openEHR should own one conformance suite in its own repositories, rather than the current unowned split across the Foundation's stalled `specifications-CNF`, Pazos's framework, and Veredictum. If that lands, it supersedes C12 as the primary oracle; C12's per-instrument runs stay useful as historical cross-checks and as evidence should the Foundation's suite adopt cases from either.
